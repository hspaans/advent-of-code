...
- The third-smallest number in both lists is 3, so the distance between them
  is 0. The next numbers to pair up are 3 and 4, a distance of 1.
...
def main() -> None:
    ...
    Read the input file and calculate the total distance and similarity score
    between the left and right lists. Print the result.
    """
    with open("input.txt", encoding="UTF-8") as f:
        ...
        similarity = sum(left.count(a) * a for a in right)
        print(f'Distance bwteen {distance} and similarity score {similarity}')
