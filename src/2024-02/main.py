...
def is_safe_report1(report: List[int]) -> bool:
...
def is_safe_report2(report: List[int]) -> bool:
    """Check if a report is safe."""
    violations = 0
    if len(report) < 2:
        print(f'Report {report} is too short')
        return False
    if report[0] < report[1]:
        for i in range(len(report) - 1):
            if report[i] > report[i + 1]:
                # print(f'Report {report} is not increasing: {report[i]} >= {report[i + 1]}')
                violations += 1
                # return False
    else:
        for i in range(len(report) - 1):
            if report[i] < report[i + 1]:
                # print(f'Report {report} is not decreasing: {report[i]} <= {report[i + 1]}')
                violations += 1
                # return False
    for i in range(1, len(report)):
        if abs(report[i - 1] - report[i]) > 3:
            # print(f'Report {report} has a difference greater than 3: {report[i - 1]} - {report[i]} = {abs(report[i - 1] - report[i])}')
            # violations += 1
            return False
    if violations > 1:
        print(f'Report {report} is unsafe ({violations} violations)')
        return False
    print(f'Report {report} is safe ({violations} violations)')
    return True


def parse_input(input_str: str) -> Tuple[List[List[int]]]:
    """Parse the input string and return a list of reports."""
    return [[int(level) for level in line.split()] for line in input_str.strip().split('\n')]
...
def main() -> None:
    ...
    with open("input2.txt", encoding="UTF-8") as f:
        ...
        safe_reports1 = sum(is_safe_report1(report) for report in reports)
        safe_reports2 = sum(is_safe_report2(report) for report in reports)
        print(f'The number of safe reports is {safe_reports1} and after dampening {safe_reports2}')
